...
import { InsertOrderTracking, InsertUser, orderTracking, users } from "../drizzle/schema";
...
export async function findOrderTracking(input: OrderLookup) {
  const db = await getDb();
...
export async function upsertOrderTracking(input: InsertOrderTracking) {
  const db = await getDb();
